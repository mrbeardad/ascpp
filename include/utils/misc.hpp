...
constexpr auto utf_conv(std::basic_string_view<FromCharT> from) -> std::basic_string<ToCharT> {
  return ww898::utf::conv<ToCharT>(from);
}

template <typename ToCharT, typename FromCharT>
constexpr auto utf_conv(const std::basic_string<FromCharT>& from) -> std::basic_string<ToCharT> {
  return ww898::utf::conv<ToCharT>(from);
}

template <typename ToCharT, typename FromCharT>
constexpr auto utf_conv(const FromCharT* from) -> std::basic_string<ToCharT> {
  return ww898::utf::conv<ToCharT>(std::basic_string_view<FromCharT>(from));
}

constexpr auto display_width(char32_t ucs) -> std::optional<int> {
  // sorted list of non-overlapping intervals of non-spacing characters
  // generated by "uniset +cat=Me +cat=Mn +cat=Cf -00AD +1160-11FF +200B c"
  constexpr std::pair<int, int> combining[]{
  ...
  auto bsearch = [](wchar_t ucs, const std::pair<int, int>* table, int max) {
    int min = 0;
    int mid;

    if (ucs < table[0].first || ucs > table[max].second)
      return 0;
    while (max >= min) {
      mid = (min + max) / 2;
      if (ucs > table[mid].second) {
        min = mid + 1;
  ...
  if (bsearch(ucs, combining, sizeof(combining) / sizeof(std::pair<int, int>) - 1)) {
    return 0;
  }
...
constexpr auto display_width(std::u32string_view str) -> std::optional<size_t> {
  auto ret = 0UZ;
  for (auto u : str) {
...
constexpr auto display_width(std::string str) -> std::optional<size_t> {
  return display_width(utf_conv<char32_t>(str));
}

constexpr auto display_width(std::wstring str) -> std::optional<size_t> {
  return display_width(utf_conv<char32_t>(str));
}

constexpr auto display_width(std::u16string str) -> std::optional<size_t> {
  return display_width(utf_conv<char32_t>(str));
}
